...
- `live_cycle.py`: 실행 시작 시 KIS에서 전략 주기에 맞는 바를 수집해 CSV로 저장한 뒤, 전략 -> 리스크 -> 주문 -> 알림까지 한 번의 운영 사이클을 실행합니다. 기본적으로 저장소 루트의 `.env`를 읽고, 템플릿은 `docs/live_cycle.env.example`에 있습니다. 다른 설정 파일은 `--env-file`로 지정할 수 있습니다.
